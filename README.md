...
## Agents Examples Reports

Example reports generated by the agents can be found in the `agent-results/` directory. These reports include details on the actions taken by the agents, such as generated locators, test cases, and triage outcomes.

These reports are generated by the agents directly as they are instructed in agents instructions file.

Agents will automatically save their results in the `agent-results/` directory, which can be reviewed to understand the actions taken and the outcomes of those actions.
